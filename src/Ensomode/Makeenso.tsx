...
import EnsoOneLine from "./EnsoOneLine";
...
export const Makeenso = () => {
  ...
  useEffect(() => {
    async function initializeData() {
      const colorlist = [12, 142, 64, 12]; //purple,teal,gray,blackのマス数
      const initSolData = await makeInit(full, colorlist, agent);
      console.log(initSolData[0]);
      setSolList(initSolData[0]);
      console.log(initSolData[1]);
  ...
  const onClickCrossOver = (
    ...
    if (bestValue < calFittness(bestTwoChild[0])) {
